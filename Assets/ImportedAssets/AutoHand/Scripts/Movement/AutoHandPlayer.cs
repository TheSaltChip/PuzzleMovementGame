...
namespace Autohand
{
    ...
    public class AutoHandPlayer : MonoBehaviour
    {
        ...
        public virtual void Start()
        {
            ...
            {
                if (editorSelected && Selection.activeGameObject == null) Selection.activeGameObject = gameObject;
            };
#endif
        }
        ...
        {
            if (grab == null || grab.HeldCount() != 0) return;
        ...
        protected virtual void UpdateTrackedObjects()
        {
            ...
            if (headPhysicsFollower != null && isGrounded)
            {
                    ...
                    headPhysicsFollower.headCollider.radius / 1.5f)
                {
                    var idealPos = headPhysicsFollower.transform.position +
                                   (headCamera.transform.position - headPhysicsFollower.transform.position).normalized *
                                   headPhysicsFollower.headCollider.radius / 1.5f;
                         ...
                         QueryTriggerInteraction.Ignore))
            {
                if (handRight.holdingObj == null || (hitRight.rigidbody != handRight.holdingObj.body &&
                                                     !handRight.holdingObj.jointedBodies.Contains(hitRight.rigidbody)))
                    if (handLeft.holdingObj == null || (hitRight.rigidbody != handLeft.holdingObj.body &&
                                                        !handLeft.holdingObj.jointedBodies
                                                            .Contains(hitRight.rigidbody)))
                        handRight.transform.position -= deltaHandPos;
            }
                         ...
                         QueryTriggerInteraction.Ignore))
            {
                if (handRight.holdingObj == null || (hitLeft.rigidbody != handRight.holdingObj.body &&
                                                     !handRight.holdingObj.jointedBodies.Contains(hitLeft.rigidbody)))
                    if (handLeft.holdingObj == null || (hitLeft.rigidbody != handLeft.holdingObj.body &&
                                                        !handLeft.holdingObj.jointedBodies.Contains(hitLeft.rigidbody)))
                        handLeft.transform.position -= deltaHandPos;
            }
        ...
        void SyncBodyHead()
        {
            ...
            if (Vector3.Distance(flatHeadPos, flatBodyPos) > 0.05f * delta)
            {
                ...
                if (!Physics.CheckCapsule(
                        direction * (0.1f * delta) + scale * transform.position +
                        Vector3.up * (scale * bodyCapsule.radius),
                        direction * (0.1f * delta) + transform.position - Vector3.up * (scale * bodyCapsule.radius) +
                        Vector3.up * (scale * bodyCapsule.height) + Vector3.up * maxStepHeight / 2f,
                ...
                else
                {
                    for (int y = -80; y <= 80; y += 40)
                    {
                        ...
                        if (!Physics.CheckCapsule(
                                newDirection * (0.1f * delta) + scale * transform.position +
                                Vector3.up * (scale * bodyCapsule.radius),
                                newDirection * (0.1f * delta) + transform.position -
                                Vector3.up * (scale * bodyCapsule.radius) + Vector3.up * (scale * bodyCapsule.height),
        ...
        protected virtual void Ground()
        {
            ...
            if (tempDisableGrounding || !useGrounding || IsClimbing() || pushAxis.y > 0) return;
            ...
            for (int i = 0; i < groundHits.Length; i++)
            {
                ...
                if (hit.collider == bodyCapsule) continue;
                if (!(hit.point.y >= point2.y) || !(hit.point.y <= point2.y + maxStepHeight)) continue;

                stepAngle = Vector3.Angle(hit.normal, Vector3.up);
                dist = hit.point.y - transform.position.y;

                if (!(stepAngle < maxStepAngle) || !(dist > highestPoint)) continue;
        ...
        protected virtual void UpdatePlayerHeight()
        {
            ...
            if (!autoAdjustColliderHeight) return;
        ...
        {
            if (!isGrounded
                || newClosestHit.transform == null
                || platformingLayerMask != (platformingLayerMask | (1 << newClosestHit.collider.gameObject.layer)))
                return;
            ...
            else if (newClosestHit.transform.Equals(closestHit.transform))
            {
                if (closestHit.transform.position == lastPlatformPosition &&
                    closestHit.transform.rotation.eulerAngles == lastPlatformRotation.eulerAngles) return;
        ...
        protected virtual void StartPush(Hand hand, GameObject other)
        {
            ...
            if (!other.CanGetComponent(out Pushable push) || !push.enabled) return;
            
            switch (hand.left)
            {
                case true when !pushLeft.ContainsKey(push):
                ...
                case true:
                    pushLeftCount[push]++;
                    break;
                case false when !pushRight.ContainsKey(push):
                {
        ...
        protected virtual void StopPush(Hand hand, GameObject other)
        {
            ...
            if (!other.CanGetComponent(out Pushable push)) return;
        ...
        protected virtual void StartGrabPush(Hand hand, Grabbable grab)
        {
            ...
            if (!grab.CanGetComponent(out Pushable push) || !push.enabled) return;
        ...
        {
            if (grab == null || !grab.CanGetComponent(out Pushable push)) return;
        ...
        protected virtual void ApplyPushingForce()
        {
            ...
            if (!allowBodyPushing) return;
            
            foreach (var push in pushRight)
            {
                if (!push.Key.enabled || push.Value.IsGrabbing()) continue;
        ...
        protected virtual void ApplyClimbingForce()
        {
            ...
            if (!allowClimbing || climbing.Count <= 0) return;
            
            foreach (var hand in climbing)
            {
                if (!hand.Value.enabled) continue;
